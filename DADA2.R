...
# Rscript --vanilla '/home/jklopp/bin/R/DADA2_without_tree_argu.R'
# -p "/media/jklopp/HD1/16s_analyse/Other_studies_as_reference/Chinese_Meconium_study_PRJNA530088/Sequences"
# -o test -r 5
...
#path <- "/media/jklopp/HD1/16s_analyse/11_20102020_11thRun"
path <- opt$inpath
print(path)
setwd(path)
#out_path <- "/media/jklopp/HD1/16s_analyse/11_20102020_11thRun"
...
#taxa <- assignTaxonomy(seqtab.nochim, "//Users/rebecca/Documents/Forschung/16S_Pipeline_Mz/silva_nr_v132_train_set.fa.gz", multithread = TRUE)
#taxa <- addSpecies(taxa, "/media/jklopp/HD1/16s_analyse/DADA2_silva_database/silva_species_assignment_v132.fa.gz")

taxa <- assignTaxonomy(seqtab.nochim, "/Users/rebecca/Documents/Forschung/16S_Pipeline_Mz/silva_nr99_v138.1_train_set.fa.gz", multithread = TRUE)
taxa <- addSpecies(taxa, "/Users/rebecca/Documents/Forschung/16S_Pipeline_Mz/silva_species_assignment_v138.1.fa.gz")


# RDP Assignemnt
#taxa <- assignTaxonomy(seqtab.nochim, "/media/jklopp/HD1/16s_analyse/DADA2_greengenes_database/rdp_train_set_16.fa.gz", multithread = TRUE)
#taxa <- addSpecies(taxa, "/media/jklopp/HD1/16s_analyse/DADA2_greengenes_database/rdp_species_assignment_16.fa.gz")
...
#------------------------------------- DECONTAM ------------------------------------------------------------
# decontam is a package to incorporate information from negative controls
# https://benjjneb.github.io/decontam/vignettes/decontam_intro.html
#rm(list=ls())
library("tidyverse"); packageVersion("tidyverse")
library("decontam"); packageVersion("decontam")

#otu_table_file <- read.csv("/media/jklopp/HD1/16s_analyse/03_16102018_3rdTestrun/ASV_table.csv", sep = "\t")
#tax_file <- read.csv("/media/jklopp/HD1/16s_analyse/03_16102018_3rdTestrun/ASV_taxonomy.csv", sep = "\t")

# if run idependently
#path <- "/Users/rebecca/Documents/Forschung/PRIMAL/16S_data/RUN_Summary"
#setwd(path)
#ps <- readRDS(paste(path,"12_DADA2_phyloseqobject.rds",sep="/"))
#otu <- otu_table(ps)
#tax <- tax_table(ps)
# if tree exisits
#tree <- phy_tree(ps)
#ps <- merge_phyloseq(otu,tax,tree,sample_data)
# else
#ps <- merge_phyloseq(otu,tax,sample_data)


metadata <- read.csv(metadata_file, fill = TRUE, sep = "\t")
# filter out rows with no sample sequenced i.e. X.SampleID is empty
...
# remove Archae and Eukaryota sequences and all bacteria that have no classification beyond kingdom
...
# threshold indicates how confident result has to be statistically (0.05 menas random chance produced the result with a propabilty below 5%)
