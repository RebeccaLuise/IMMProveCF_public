...
ps_full_sputum_v1 <- subset_samples(ps_full_sputum, visit== 1)

ps_full_sputum_rep <- subset_samples(ps_full_sputum, id.x%in% c(11,21,24,25,26,27,29,33,5,6,9))
ps_full_sputum_v1 <- subset_samples(ps_full_sputum_rep, visit== 1)
ps_full_sputum_v1 <- tax_filter(ps_full_sputum_v1, min_prevalence = 0.25)

tax_v1 <- as.data.frame(tax_table(ps_full_sputum_v1))

tax_v1_genera <- unique(tax_v1$Genus) # lists 41 taxa

# Subset the full dataset to include only the taxa retained in the filtered dataset
ps_full_sputum_v1_taxa <- prune_taxa(taxa_names(ps_full_sputum) %in% taxa_names(ps_full_sputum_v1), ps_full_sputum)


ps_sputum_relab <- transform_sample_counts(ps_full_sputum_v1_taxa, function(x) x/sum(x))
...
effect_table_sig <- effect_table%>%
  ...
  filter(q_V1_V2<0.1|q_V1_V3V5<0.1|q_V1_V6V7<0.1|q_V1_V8V9<0.1)
...
```{r plot cuneiform }
sign(effect_table_sig_long$effectSize)
...
```{r Combine different tax levels into one plot}
...
p1 <- effect_table_sig_all%>%
  ...
  scale_shape_manual (name="",labels=c("decreased","increased"), values = c (25,0, 24)) + 
...
```{r combine sputum and stool results}
