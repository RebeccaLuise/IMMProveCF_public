...
title: "Calculate Effect size for visits, comparison healthy controls vs all CF timepoints - throat"
...
sample_data(ps_throat)$visit_sum_ordered <- factor(sample_data(ps_throat)$visit_sum, levels = c("1", "2", "3-5", "6-7", "8-10"), ordered = TRUE)# Convert visit_sum to an ordered factor
is.ordered(sample_data(ps_throat)$visit_sum_ordered)
...
ps_throat_relab_genus <- tax_glom(ps_throat_relab, "Genus")
...
ps_cf_z_phylum <- calculate_z_scores(ps_throat_relab_phylum)
ps_cf_z_class <- calculate_z_scores(ps_throat_relab_class)
ps_cf_z_order <- calculate_z_scores(ps_throat_relab_order)
ps_cf_z_family <- calculate_z_scores(ps_throat_relab_family)
ps_cf_z_genus <- calculate_z_scores(ps_throat_relab_genus)
...
# Example usage
ps_r_z_phylum <- calculate_mad_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Phylum")
ps_r_z_df_phylum <- psmelt(ps_r_z_phylum)

ps_r_z_genus <- calculate_mad_z_scores(physeq_object = ps_throat_relab, 
...
# Example usage
ps_iqr_z_phylum <- calculate_iqr_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Phylum")
ps_iqr_z_df_phylum <- psmelt(ps_iqr_z_phylum)

ps_iqr_z_order <- calculate_iqr_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Order")
ps_iqr_z_df_order <- psmelt(ps_iqr_z_order)

ps_iqr_z_family <- calculate_iqr_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Family")
ps_iqr_z_df_family <- psmelt(ps_iqr_z_family)

ps_iqr_z_class <- calculate_iqr_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Class")
ps_iqr_z_df_class <- psmelt(ps_iqr_z_class)

ps_iqr_z_genus <- calculate_iqr_z_scores(physeq_object = ps_throat_relab, 
                                         taxonomic_rank = "Genus")
...
```{r}
# Subset phyloseq object to get healthy controls
healthy_samples <- subset_samples(ps_throat_relab_phylum, project == "IMMPIMMP")
...
# Subset phyloseq object to get CF samples
cf_samples <- subset_samples(ps_throat_relab_phylum, project == "IMP")
...
# Usage of the function on ps_throat_relab
directional_percentiles <- calculate_directional_percentiles(ps_throat_relab)
...
write_csv(all_stats_iqr, "/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/Revision/SuppMaterial/iqr_zscore_throat.csv")

```

```{r}
throat_sig_baseline_control <- read_excel("/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/IMMProveCF_public/throat_sigComp_p0.05_BaselineVsControl.xlsx")

# now filter stats for significant taxa that were observed in throat samples 
all_stats_iqr_filtered_taxa <- all_stats_iqr %>% 
  filter(Taxa %in% throat_sig_baseline_control$Taxa) %>% 
  distinct(Taxa) %>% 
  left_join(throat_sig_baseline_control, by="Taxa")

view(all_stats_iqr_filtered_taxa)

tax <- as.data.frame(tax_table(ps_throat_relab))
...
tax_phylum <- tax %>% 
  filter(Phylum %in% all_stats_iqr_filtered_taxa$Taxa)

tax_all <- rbind(tax_phylum, tax_class,tax_order, tax_fam, tax_genus)
...
boxplot <- taxa_data %>%
  ...
  guides(fill = 'none') +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  stat_pvalue_manual(lm_stats, label = "fdr_star", y.position = y_pos, step.increase = 0.05)
...
results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_genus, taxonomic_rank = "Genus", taxa_to_run = c("Rothia"), y_pos=10)
bp1 <- results$boxplots$Rothia #+scale_y_log10()
lm1 <- results$lm_stats$Rothia

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_genus, taxonomic_rank = "Genus", taxa_to_run = c("Actinomyces"), y_pos=10)
bp2 <- results$boxplots$Actinomyces
lm2 <- results$lm_stats$Actinomyces

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_genus, taxonomic_rank = "Genus", taxa_to_run = c("Atopobium"), y_pos=10)
bp6 <- results$boxplots$Atopobium
lm6 <- results$lm_stats$Atopobium

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_genus, taxonomic_rank = "Genus", taxa_to_run = c("Lautropia"), y_pos=4)
bp3 <- results$boxplots$Lautropia
lm3 <- results$lm_stats$Lautropia

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_genus, taxonomic_rank = "Genus", taxa_to_run = c("Stomatobaculum"), y_pos=3)
bp4 <- results$boxplots$Stomatobaculum
lm4 <- results$lm_stats$Stomatobaculum

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_family, taxonomic_rank = "Family", taxa_to_run = c("Burkholderiaceae"), y_pos=0.5)
bp5 <- results$boxplots$Burkholderiaceae 
lm5 <- results$lm_stats$Burkholderiaceae 


results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_phylum, taxonomic_rank = "Phylum", taxa_to_run = c("Proteobacteria"), y_pos =2)
...
results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_phylum, taxonomic_rank = "Phylum", taxa_to_run = c("Bacteroidota"), y_pos=2.3)
bp11 <- results$boxplots$Bacteroidota
lm11 <- results$lm_stats$Bacteroidota

results <- lm_corrected_pvalues_boxplot(ps_iqr_z_df_phylum, taxonomic_rank = "Phylum", taxa_to_run = c("Actinobacteriota"), y_pos=2.3)
bp12 <- results$boxplots$Actinobacteriota
lm12 <- results$lm_stats$Actinobacteriota


# Optionally, combine all boxplots and lm_stats for further analysis or plotting
boxplots_list <- list(bp1, bp2, bp3, bp4, bp5, bp6, bp9, bp10, bp11, bp12)
lm_stats_list <- list(lm1, lm2, lm3, lm4, lm5, lm6, lm9, lm10, lm11, lm12)
...
ggarrange(bp12, bp1, ncol = 2, nrow = 1)
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/IQR-Z-scores_sig_throat.png", height = 4, width = 8)

saveRDS(bp12, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/IQR-Z-scores_sig_throat_Actinobacteriota.rds")
saveRDS(bp1, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/IQR-Z-scores_sig_throat_Rothia.rds")
```

```{r}
# Extract unique taxa for each rank
unique_phylum <- unique(tax_all$Phylum)
unique_class <- unique(tax_all$Class)
unique_order <- unique(tax_all$Order)
unique_family <- unique(tax_all$Family)
unique_genus <- unique(tax_all$Genus)

# Define a function to run tests for a given taxon and rank
run_tests_for_taxon <- function(taxa_vector, taxonomic_rank, data_frame) {
  results_list <- list()
  for (taxon in taxa_vector) {
    results <- lm_corrected_pvalues_boxplot(data_frame, taxonomic_rank = taxonomic_rank, taxa_to_run = c(taxon), y_pos=10)
    boxplot_path <- results$boxplots[[taxon]]
    lm_stats_path <- results$lm_stats[[taxon]]
    results_list[[taxon]] <- list(boxplot = boxplot_path, lm_stats = lm_stats_path)
  }
  return(results_list)
}

# Run tests for each taxonomic rank
results_phylum <- run_tests_for_taxon(unique_phylum, "Phylum", ps_iqr_z_df_phylum)
results_class <- run_tests_for_taxon(unique_class, "Class", ps_iqr_z_df_class) # Assuming ps_iqr_z_df_class is defined
results_order <- run_tests_for_taxon(unique_order, "Order", ps_iqr_z_df_order) # Assuming ps_iqr_z_df_order is defined
results_family <- run_tests_for_taxon(unique_family, "Family", ps_iqr_z_df_family)
results_genus <- run_tests_for_taxon(unique_genus, "Genus", ps_iqr_z_df_genus)

# Example of accessing results
print(results_phylum$Actinobacteriota$boxplot)
print(results_phylum$Actinobacteriota$lm_stats)

print(results_class$Actinobacteria$boxplot)
print(results_class$Actinobacteria$lm_stats)

print(results_class$Coriobacteriia$boxplot)
print(results_class$Coriobacteriia$lm_stats)

print(results_order$Micrococcales$boxplot)
print(results_order$Micrococcales$lm_stats)

print(results_order$Actinomycetales$boxplot)
print(results_order$Actinomycetales$lm_stats)

print(results_order$Coriobacteriales$boxplot)
print(results_order$Coriobacteriales$lm_stats)
...
taxa_info <- list(
  list(data = ps_iqr_z_df_genus, taxon = "Rothia", column = "Genus"),
  list(data = ps_iqr_z_df_genus, taxon = "Atopobium", column = "Genus"),
  list(data = ps_iqr_z_df_genus, taxon = "Actinomyces", column = "Genus"),
    list(data = ps_iqr_z_df_phylum, taxon = "Actinobacteriota", column = "Phylum"),
