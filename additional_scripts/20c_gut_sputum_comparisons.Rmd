---
title: "Compare stool and sputum samples, of participants who provided both sputum and stool at the same timepoint"
...
This script was used to run initial comparisons between the gut-sputum axis, none of the results are displayed in the latest manuscript version (June 20th 2025), but we refer to the negative results of the linear models between BC distances, such as that distance in sputum samples does not predict changes in stool samples, whereas there is such significant association found in the sputum_throat comparison.
...
ps_full <- ps_Sputum_Stool_full_unique_2_relab# to make loading of the function possible I have to rename the ps
source("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/IMMProveCF/functions_full.R")

ps_full_sputum_glom <- tax_glom(ps_Sputum_Stool_full_unique_2_relab, taxrank = "Genus")
...
ps_Sputum_Stool_full_unique_2_relab<- microViz::ps_join(ps_Sputum_Stool_full_unique_2_relab, top.sputum_df, by = "x1")
```

```{r Beta diversity sputum & stool samples dominant genus}

ps_Sputum_Stool_full.ord <- ordinate(ps_Sputum_Stool_full_unique_2_relab, "MDS", "bray")

plot_ordination(ps_Sputum_Stool_full_unique_2_relab, ps_Sputum_Stool_full.ord, color="dominantGenus", shape = "material", label = "visit_cal_cor")+
  geom_point(aes(label= visit), size=2.5)+
  scale_color_manual(values=dom_palette)+
  ggtitle("Beta-diversity of all sputum & stool samples provided from the same patient at the same timepoint")
```

```{r calculate bray curtis distance for all sputum & stool samples only v1, eval=FALSE}

ps_Sputum_Stool_full_unique_v1 <- subset_samples(ps_Sputum_Stool_full_unique_2, visit == "1")

ps_Sputum_Stool_full.ord_v1 <- ordinate(ps_Sputum_Stool_full_unique_v1, "MDS", "bray")

plot_ordination(ps_Sputum_Stool_full_unique_v1, ps_Sputum_Stool_full.ord_v1, color="id", shape = "material")+   #stat_ellipse(type = "t")+
scale_color_manual(values=id_palette)+
  ggtitle("Beta-diversity of sputum & stool samples provided from the same patient at visit 1")
```

# PERMANOVA
```{r calculate bray curtis distance for all sputum & stool samples and run PERMANOVA}
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_Sputum_Stool_full_unique_2_relab,
...
#sjPlot::tab_df(v1_df, title= "PERMANOVA of paired sputum/stool samples", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/permanova_paired_Sputum_Stool.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/permanova_paired_Sputum_Stool.html") # prints the html file in the same directory as a pdf
```
...
BC_dist<- phyloseq::distance(ps_Sputum_Stool_full_unique_2_relab,
                             method="bray", weighted=F)
ordination<- ordinate(ps_Sputum_Stool_full_unique_2,
                      method="PCoA", distance= BC_dist)

BC_plot <- plot_ordination(ps_Sputum_Stool_full_unique_2_relab, ordination, color="id", shape = "material")+
  geom_point(size=3)+
  ...
  #ggtitle("Beta-diversity of paired sputum/stool samples")
BC_plot
...
# compare distances between stool and sputum
```{r compare distances between stool and sputum}
...
p2 <- tmp5%>%
  ggplot(aes(visit_id_pair_y,Sputum_Stool, fill=visit_id_pair_y, alpha=0.7))+
...
lm1 <- summary(lmerTest::lmer(Sputum_Stool ~ id_pair_y + visit_pair_y + (1|id.x1) + (1|id.x2), data=tmp5))
...
#sjPlot::tab_df(lm1_df, title= "Comparison of BC distance between sputum/stool sample for same and different time points intra-/and interindividual (lmer(Sputum_Stool_BC ~ id_pair_y + visit_pair_y + (1|id.x1) + (1|id.x2))", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/lm_Sputum_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/lm_Sputum_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

# now let's see if the distance between stool samples can actually predict distance between sputum samples
tmp6 <- tmp2%>%
  mutate(id_pairs = paste(id.x1, id.x2, sep="_"))%>%
  mutate(sample_pairs = paste(Var1, Var2, sep="_"))%>%
  mutate(material_pairs = paste(material.x1, material.x2, sep="_"))%>%
...
tmp6%>%
  ggplot(aes(Stool_Stool,Sputum_Sputum,color=id_pair_y))+
...
## now the statistics
summary(lmerTest::lmer(Sputum_Sputum ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6))
summary(lmerTest::lmer(Stool_Stool ~ Sputum_Sputum + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6))
# if we predict Sputum from stool the time variable between samples has an sig impact on the prediction
# if we predict stool from Sputum, time does not matter, probably because there is no sig difference in BC dist over time 

labels <- c(`1`="Intra-id, \n different sample time points", `2`="Inter-id, \n same sample time points",`3`="Inter-id, \n  different sample time points")
p3 <- tmp6%>%
  ggplot(aes(Stool_Stool,Sputum_Sputum))+
...
lm1 <- summary(lmerTest::lmer(Sputum_Sputum ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_1))
...
#sjPlot::tab_df(lm1_df, title= "Intra-id, \n different sample time points lmer(Sputum_Sputum ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_1)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/intraID_Sputum_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/intraID_Sputum_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

#`2`="Inter-id, \n same sample time points"
tmp6_2<-tmp6%>%
  filter(visit_id_pair_y==2)

lm2 <- summary(lmerTest::lmer(Sputum_Sputum ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_2))
...
#sjPlot::tab_df(lm2_df, title= "Inter-id, same sample time points lmer(Sputum_Sputum ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_2)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_ST_Sputum_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_ST_Sputum_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

#`3`="Inter-id, \n  different sample time points"
tmp6_3<-tmp6%>%
  filter(visit_id_pair_y==3)

lm3 <- summary(lmerTest::lmer(Sputum_Sputum ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_3))
...
#sjPlot::tab_df(lm3_df, title= "Inter-id, different sample time points lmer(Sputum_Sputum ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_3)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_DT_Sputum_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_DT_Sputum_Stool_BC_dist.html") # prints the html file in the same directory as a pdf
...
## compare sputum and stool in trees; run differential testing for all visits
```{r differential tree per patient and sample time point, warning=FALSE}
ps_Sputum_Stool_full_unique_2 <- tax_filter(ps_Sputum_Stool_full_unique_2, min_prevalence = 1)

#transform to relative abundance
ps_Sputum_Stool_full_unique_2_relab <- transform_sample_counts(ps_Sputum_Stool_full_unique_2 , function(x) x/sum(x))

ps_Sputum_Stool_full_unique_2_relab_genus <- tax_glom(ps_Sputum_Stool_full_unique_2_relab, taxrank = "Genus")

tax_data <- as.data.frame(tax_table(ps_Sputum_Stool_full_unique_2_relab_genus))
...
otu_data <- as.data.frame(t(otu_table(ps_Sputum_Stool_full_unique_2_relab_genus)))%>%
  rownames_to_column("ASV_ID")

otu_data <- left_join(otu_data, tax_data,
                      by = c("ASV_ID" = "ASV_ID"))

sample_data <- as.data.frame(sample_data(ps_Sputum_Stool_full_unique_2_relab_genus))
...
#print(otu_data)
sample_data <- as_tibble(sample_data(ps_Sputum_Stool_full_unique_2_relab_genus))
#print(sample_data)
...
set.seed(999)
# taxa more abundant in Sputum are colored in green, those more abundant in stool in purple
heat_tree(obj, 
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/diff_tree stool_sputum_all_woSig.pdf", width = 10, height = 7)
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/diff_tree stool_sputum_all_Sig.pdf", tree_plot, width = 12, height = 7)
```
...
list_shared_genus <- ps_venn(ps_Sputum_Stool_full_unique_2_relab_genus, group = "material", plot = F)

library(eulerr)
# Fit a diagram with circles
combo <- c(Sputum = 41, Stool = 106, "Sputum&Stool" = 86)
fit1 <- euler(combo)
...
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/gut_sputum.png",grid_arranged, dpi = 300, width = 19, height = 24)
```