---
title: "Compare stool and throat samples, of participants who provided both throat and stool at the same timepoint"
...
output: 
  html_document:
    ...
    toc_float: yes
---
This script was used to run initial comparisons between the gut-throat axis, none of the results are displayed in the latest manuscript version (June 20th 2025), but we refer to the negative results of the linear models between BC distances, such as that distance in throat samples does not predict changes in stool samples, whereas there is such significant association found in the sputum_throat comparison.
...
ps_throat_full <- readRDS("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/rds_files/ps_IMProveCF_patients_Run1-23_18102023.rds")
...
pacman::p_load(rmarkdown, tidyverse, phyloseq, magrittr, janitor, microbiome, knitr, lubridate, naniar, readxl, ggplot2, ggpubr,rstatix, metadeconfoundR, metacoder,agricolae, ape, gridExtra, stargazer)
...
material_s_t <- c("Throat", "Stool")

ps_throat_Stool_full <- subset_samples(ps_throat_full, material %in% material_s_t)

ps_throat_full_throat <- subset_samples(ps_throat_full, material== "Throat")

id_visit_throat <- sample_data(ps_throat_full_throat)$id_visit

ps_throat_Stool_full_unique <- subset_samples(ps_throat_full, id_visit %in% id_visit_throat)#includes also stool samples
ps_throat_Stool_full_unique <- subset_samples(ps_throat_Stool_full_unique, material %in% material_s_t)#includes still throat samples which do not have a paired stool sample

# find those stool samples which have a paired throat sample
ps_throat_full_stool <- subset_samples(ps_throat_full, material== "Stool")
ps_throat_Stool_full_unique_3 <- subset_samples(ps_throat_full_stool, id_visit %in% id_visit_throat)#includes also stool samples
id_visit_unique <- sample_data(ps_throat_Stool_full_unique_3)$id_visit

#filter for only those samples which have both throat and stool from the same timepoint
ps_throat_Stool_full_unique_2 <- subset_samples(ps_throat_Stool_full_unique, id_visit %in% id_visit_unique)#includes still throat samples which do not have a paired stool sample

summary(sample_data(ps_throat_Stool_full_unique_2)$id) # 15 participants provided both throat & stool samples at the same timepoint

data <- as(sample_data(ps_throat_Stool_full_unique_2), "data.frame")

tabyl(data, id, visit_cal_cor)

#transform to relative abundance
ps_throat_Stool_full_unique_2_relab <- transform_sample_counts(ps_throat_Stool_full_unique_2 , function(x) x/sum(x))
```

# Sample overview
```{r plot sample overview, include=FALSE}
# plot sample overview
bp_ps_relab <- plot_bar(ps_throat_Stool_full_unique_2_relab, "id", fill="Phylum")
...
ps_full <- ps_throat_Stool_full_unique_2_relab# to make loading of the function possible I have to rename the ps
source("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/IMMProveCF/functions_full.R")

ps_full_throat_glom <- tax_glom(ps_throat_Stool_full_unique_2_relab, taxrank = "Genus")
...
ps_throat_Stool_full_unique_2_relab<- microViz::ps_join(ps_throat_Stool_full_unique_2_relab, top.throat_df, by = "x1")
```

```{r Beta diversity throat & stool samples dominant genus}

ps_throat_Stool_full.ord <- ordinate(ps_throat_Stool_full_unique_2_relab, "MDS", "bray")

plot_ordination(ps_throat_Stool_full_unique_2_relab, ps_throat_Stool_full.ord, color="dominantGenus", shape = "material", label = "visit_cal_cor")+
  geom_point(aes(label= visit), size=2.5)+
  scale_color_manual(values=dom_palette)+
  ggtitle("Beta-diversity of all throat & stool samples provided from the same patient at the same timepoint")
```

```{r calculate bray curtis distance for all throat & stool samples only v1, eval=FALSE}
ps_throat_Stool_full_unique_v1 <- subset_samples(ps_throat_Stool_full_unique_2, visit == "1")
ps_throat_Stool_full.ord_v1 <- ordinate(ps_throat_Stool_full_unique_v1, "MDS", "bray")

plot_ordination(ps_throat_Stool_full_unique_v1, ps_throat_Stool_full.ord_v1, color="id", shape = "material")+   #stat_ellipse(type = "t")+
scale_color_manual(values=id_palette)+
  ggtitle("Beta-diversity of throat & stool samples provided from the same patient at visit 1")
```

# PERMANOVA
```{r calculate bray curtis distance for all throat & stool samples and run PERMANOVA}
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_throat_Stool_full_unique_2_relab,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_throat_Stool_full_unique_2_relab),"data.frame")

#vegan::adonis2(Bproject#vegan::adonis2(BC_dist ~ material + visit + id,
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/paired_variance_explained.pdf", width = 5, height = 3)
...
#sjPlot::tab_df(v1_df, title= "PERMANOVA of paired throat/stool samples", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/permanova_paired_throat_Stool.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/permanova_paired_throat_Stool.html") # prints the html file in the same directory as a pdf
```
...
BC_dist<- phyloseq::distance(ps_throat_Stool_full_unique_2_relab,
                             method="bray", weighted=F)
ordination<- ordinate(ps_throat_Stool_full_unique_2,
                      method="PCoA", distance= BC_dist)

BC_plot <- plot_ordination(ps_throat_Stool_full_unique_2_relab, ordination, color="id", shape = "material")+
  geom_point(size=3)+
  geom_line(aes(group=id_visit), size=1.5, alpha=0.2)+
  scale_color_manual(values=id_palette)+
  #geom_text(aes(label=visit), hjust=1.5, vjust=1.5)+
  theme_classic()+
  theme(text=element_text(size = 17))#+
  #ggtitle("Beta-diversity of paired throat/stool samples")
BC_plot

#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/Beta_diversityThrSpu_conLine.pdf", BC_plot, width = 8.5, height = 5)
```

# compare distances between stool and throat
```{r compare distances between stool and throat}
...
tmp1 <- BC_dist.throat_df%>%left_join(metadata, by=c("Var1"="x_sample_id"))%>%
...
p2 <- tmp5%>%
  ggplot(aes(visit_id_pair_y,Throat_Stool, fill=visit_id_pair_y, alpha=0.7))+
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/IntraID_BCdist.pdf",p2,  width = 8, height = 7)
...
lm1 <- summary(lmerTest::lmer(Throat_Stool ~ id_pair_y + visit_pair_y + (1|id.x1) + (1|id.x2), data=tmp5))
...
#sjPlot::tab_df(lm1_df, title= "Comparison of BC distance between throat/stool sample for same and different time points intra-/and interindividual (lmer(throat_Stool_BC ~ id_pair_y + visit_pair_y + (1|id.x1) + (1|id.x2))", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/lm_throat_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/lm_throat_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

# now let's see if the distance between stool samples can actually predict distance between throat samples
tmp6 <- tmp2%>%
  mutate(id_pairs = paste(id.x1, id.x2, sep="_"))%>%
  mutate(sample_pairs = paste(Var1, Var2, sep="_"))%>%
  mutate(material_pairs = paste(material.x1, material.x2, sep="_"))%>%
...
tmp6%>%
  ggplot(aes(tt_dif_days, Stool_Stool, color=id_pair_y))+
  geom_point()+
...
tmp6%>%
  ggplot(aes(Stool_Stool,Throat_Throat,color=id_pair_y))+
  geom_point(aes(color=id_pair_y))+
  stat_smooth(method = "lm")+
  stat_cor(method = "spearman")+
  theme_bw()+
  scale_color_manual(values = c("#7CA2D4","#FDA9DF"), labels= c("Inter-individual", "Intra-individual"))+
  ylab("BC distance between throat sample pairs")+
  theme(legend.position = "bottom", legend.title = element_blank(), text=element_text(size=16))+
  xlab("BC distance between stool sample pairs")

##gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/BCdist_pairs.pdf", width = 10, height = 7)
...
## now the statistics
summary(lmerTest::lmer(Throat_Throat ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6))
summary(lmerTest::lmer(Stool_Stool ~ Throat_Throat + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6))
# if we predict throat from stool the time variable between samples has an sig impact on the prediction
# if we predict stool from throat, time does not matter, probably because there is no sig difference in BC dist over time 

labels <- c(`1`="Intra-id, \n different sample time points", `2`="Inter-id, \n same sample time points",`3`="Inter-id, \n  different sample time points")
p3 <- tmp6%>%
  ggplot(aes(Stool_Stool,Throat_Throat))+
...
p6 <- tmp6 %>%
  ...
  labs(color = "Delta in tt days btwn sample time points")
p6
##gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/Correlation_BCdist_groups.pdf", width = 9, height = 6)
...
lm1 <- summary(lmerTest::lmer(Throat_Throat ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_1))
...
#sjPlot::tab_df(lm1_df, title= "Intra-id, \n different sample time points lmer(Throat_Throat ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_1)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/intraID_throat_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/intraID_throat_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

#`2`="Inter-id, \n same sample time points"
tmp6_2<-tmp6%>%
  filter(visit_id_pair_y==2)

lm2 <- summary(lmerTest::lmer(Throat_Throat ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_2))
...
#sjPlot::tab_df(lm2_df, title= "Inter-id, same sample time points lmer(Throat_Throat ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_2)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_ST_throat_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_ST_throat_Stool_BC_dist.html") # prints the html file in the same directory as a pdf

#`3`="Inter-id, \n  different sample time points"
tmp6_3<-tmp6%>%
  filter(visit_id_pair_y==3)

lm3 <- summary(lmerTest::lmer(Throat_Throat ~ Stool_Stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_3))
...
#sjPlot::tab_df(lm3_df, title= "Inter-id, different sample time points lmer(Throat_Throat ~ stool_stool + tt_dif_days + (1|id_pairs)+ (1|visit_pairs), data=tmp6_3)", file = "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_DT_throat_Stool_BC_dist.html")

#pagedown::chrome_print("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/table_outputs/interID_DT_throat_Stool_BC_dist.html") # prints the html file in the same directory as a pdf
...
## compare throat and stool in trees; run differential testing for all visits
```{r differential tree per patient and sample time point, warning=FALSE}
ps_throat_Stool_full_unique_2 <- tax_filter(ps_throat_Stool_full_unique_2, min_prevalence = 1)

#transform to relative abundance
ps_throat_Stool_full_unique_2_relab <- transform_sample_counts(ps_throat_Stool_full_unique_2 , function(x) x/sum(x))

ps_throat_Stool_full_unique_2_relab_genus <- tax_glom(ps_throat_Stool_full_unique_2_relab, taxrank = "Genus")

tax_data <- as.data.frame(tax_table(ps_throat_Stool_full_unique_2_relab_genus))
...
otu_data <- as.data.frame(t(otu_table(ps_throat_Stool_full_unique_2_relab_genus)))%>%
  rownames_to_column("ASV_ID")

otu_data <- left_join(otu_data, tax_data,
                      by = c("ASV_ID" = "ASV_ID"))

sample_data <- as.data.frame(sample_data(ps_throat_Stool_full_unique_2_relab_genus))
...
#print(otu_data)
sample_data <- as_tibble(sample_data(ps_throat_Stool_full_unique_2_relab_genus))
#print(sample_data)
...
set.seed(999)
# taxa more abundant in throat are colored in green, those more abundant in stool in purple
heat_tree(obj, 
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/diff_tree stool_throat_all_woSig.pdf", width = 10, height = 7)
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/diff_tree stool_throat_all_Sig.pdf", tree_plot, width = 12, height = 7)
```
...
list_shared_genus <- ps_venn(ps_throat_Stool_full_unique_2_relab_genus, group = "material", plot = F)

library(eulerr)
# Fit a diagram with circles
combo <- c(Throat = 101, Stool = 93, "Throat&Stool" = 144)
fit1 <- euler(combo)
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/Venn_all_sputhr.pdf", venn1, width = 7, height = 5)
...
grid_arranged <- grid.arrange(NULL,
...
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/gut_throat.png",grid_arranged, dpi = 300, width = 19, height = 24)
```