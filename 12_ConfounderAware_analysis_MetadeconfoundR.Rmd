...
geom_tile(aes(fill = Ds), color = "black") + 
  ...
  theme(axis.text.x=element_text(size=14, angle = 40, hjust = 1), axis.text.y=element_text(size=14), legend.position = "right")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)

tab_sputum_2 <- BuildHeatmap(sputum_metad_div,intermedData = T,showConfounded = F, q_cutoff = 0.1, d_cutoff = 0.1)

sputum_div_hm <- tab_sputum_2 %>% 
  ggplot(aes(x=metaVariable,y=feature,fill=Ds))+
  geom_tile() +
  theme_minimal(base_size = 14)+
geom_tile(aes(fill = Ds), color = "black") + 
            scale_fill_gradient2(name= "Effect size", low = "#746999", high = "#69b3a2", mid = "white", midpoint = 0, guide = guide_colorbar(raster = F), 
                limits = c(min(sputum_metad_div$Ds), max(sputum_metad_div$Ds))) + 
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=14, angle = 40, hjust = 1), axis.text.y=element_text(size=14), legend.position = "bottom", legend.text = element_text(size=12))+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)

saveRDS(sputum_div_hm, "/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/IMMProveCF_public/outputs/sputum_div_hm.rds")
...
geom_tile(aes(fill = Ds), color = "black") + 
            scale_fill_gradient2(name= "Effect size", low = "#746999", high = "#69b3a2", mid = "white", midpoint = 0, guide = guide_colorbar(raster = F), 
                limits = c(min(sputum_metad_div$Ds), max(sputum_metad_div$Ds))) + 
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=12, angle = 40, hjust = 1), axis.text.y=element_text(size=12), legend.position = "right")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)
...
geom_tile(aes(fill = Ds), color = "black") + 
            scale_fill_gradient2(name= "Effect size (Cliff's delta)", low = "#746999", high = "#69b3a2", mid = "white", midpoint = 0, guide = guide_colorbar(raster = F), 
                limits = c(min(sputum_metad_div$Ds), max(sputum_metad_div$Ds))) + 
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=10, angle = 40, hjust = 1), axis.text.y=element_text(size=10), legend.position = "right")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)+
...
geom_tile(aes(fill = Ds), color = "black") + 
  ...
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=10, angle = 40, hjust = 1, vjust = 1.05), axis.text.y=element_text(size=10), legend.title = element_text(size=10), legend.position = "bottom")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)+
...
geom_tile(aes(fill = Ds), color = "black") + 
            scale_fill_gradient2(name= "Effect size (Cliff's delta)", low = "#746999", high = "#69b3a2", mid = "white", midpoint = 0, guide = guide_colorbar(raster = F), 
                limits = c(min(stool_metad_div$Ds), max(stool_metad_div$Ds))) + 
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=10, angle = 40, hjust = 1), axis.text.y=element_text(size=1), legend.position = "right")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels)
...
```
### atb treatment days and N observed ASVs
```{r}
visit_sum_palette <- c("black", "#C3BC3FFF","#6388B4FF","#BB7693FF","#55AD89FF") 
x2labels <- c("0", "3", "6-12", "15-18", "21-24")

atbx_obs <- diversity_df_stool_metadata %>% 
  ggplot(aes(x = treatmentdays_365prior_visit, y=observed))+
...
atbx_obs
saveRDS(atbx_obs, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/rds_files/atbx_obs.rds")
...
hm_05 <- hm_05 %>%
  mutate(feature = case_when(
    grepl("\\.", featureNames) ~ gsub("\\.", " ", featureNames),
    TRUE ~ featureNames  # if no dot, keep the original
  ))%>%
  mutate(feature = gsub("X Eubacterium  nodatum group", "Eubacterium nodatum group", feature)) %>% 
  mutate(feature = gsub("Prevotella_7", "Prevotella (7)", feature))
...
geom_tile(aes(fill = Ds), color = "black") + 
  ...
  geom_text(aes(label=stars), color = "black", size = 5)+
  theme(axis.text.x=element_text(size=10, angle = 40, hjust = 1, vjust = 1.05), axis.text.y=element_text(size=10, face="italic"), legend.text = element_text(size=12), legend.position = "bottom")+
  xlab("")+
  ylab("")+
  scale_x_discrete(labels = xlabels)+
  scale_y_discrete(labels = ylabels, position = "right")
sputum_hm
...
p1 <- ggarrange(igg_leuco, pseudomonas, pseu_igg,pseu_ab, leu_pn, igg_pn, labels = c("a","b","c","d","e","f"), nrow = 2, ncol = 3, legend = "none", common.legend = T)

ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/pseudo_inflammation_new.png",p1, width = 10, height = 7)

ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/Revision/pdf_figures/supp_figure3_pseudo_inflammation.pdf",p1, width = 12, height = 8)

Legend_c <- ggarrange(legend1, legend2)

ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/Revision/pdf_figures/pseudo_inflammation_legend.pdf", Legend_c, width = 12, height = 2)
...
```{r arrange plots for figure 4, fig.height=13, fig.width=11, eval=FALSE}
library(gridExtra)
...
throat_hm <- hm_05 %>%
  ...
  mutate(feature = gsub("X Eubacterium  brachy group", "Eubacterium brachy group", feature)) %>% 
  mutate(feature = gsub("X Eubacterium  nodatum group", "Eubacterium nodatum group", feature)) %>%
  mutate(feature = gsub("Prevotella_7", "Prevotella (7)", feature)) %>% 
  filter(metaVariable!="colistin_inhal") %>% 
  ggplot(aes(x=metaVariable,y=feature,fill=Ds))+
...
stool_hm <- hm_05 %>% 
  ...
  mutate(feature = gsub("X Eubacterium  nodatum group", "Eubacterium nodatum group", feature)) %>% 
  mutate(feature = gsub("X Ruminococcus  torques group", "Ruminococcus torques group", feature)) %>%
  mutate(feature = gsub("X Eubacterium  eligens group", "Eubacterium eligens group", feature)) %>%
...
metadata_x_stool %>% 
  ggplot(aes(as_factor(cortison_oral), calprotectin_amg_g_stuhl))+
  geom_boxplot()+
...
# problematic here is that calprotectin measurements from patients that haven taken oral cortisone are only available from 2 patients, while 3 patients had taken cortisone_oral in the total cohort, 3 was my cutoff to investigate the variable as it get's close to 10% of the population studied. 
```
```{r}
glimpse(metadata)
```
