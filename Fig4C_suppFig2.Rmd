...
```{r plot cuneiform with all differences from Controls to baseline class, eval=FALSE}
# select the entries which have OK_nc in status with all differences to Control
...
```{r plot cuneiform with all differences from Controls to baseline phylum, eval=FALSE}
# select the entries which have OK_nc in status with all differences to Control
...
p1_control_fdr <- effect_table_sig_all_controls_fdr %>%
  mutate(shape_sign = as.factor(sign(effectSize))) %>% 
  filter(shape_sign!=0) %>% 
  #mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  #mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
