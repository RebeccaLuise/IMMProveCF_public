---
title: "Taxonomic differential abundance testing for stool samples compared to CF baseline (Visit 1)"
...
# subset stool providing patients

stool_id <- c("IMP11",  "IMP13", "IMP15", "IMP16", "IMP17", "IMP24", "IMP25", "IMP26", "IMP27", "IMP29", "IMP31", "IMP32", "IMP33", "IMP35", "IMP5",  "IMP6", "IMP9",  "IMP21", "IMP8")

stool_palette <- c(IMP11="#4E79A7FF", IMP13="#A0CBE8FF",IMP15="#F28E2BFF",IMP16="#FFBE7DFF",IMP17="#59A14FFF", IMP24="#8CD17DFF", IMP25="#B6992DFF",IMP26="#F1CE63FF",IMP27="#499894FF", IMP29="#86BCB6FF",IMP31="#D9D9D9", IMP32="#E15759FF",  IMP33="#FF9D9AFF",  IMP35="#79706EFF",IMP5="#BAB0ACFF", IMP6="#D37295FF", IMP9="#E5C494", IMP21="#B07AA1FF", IMP8="black")
```
...
ps_stool_relab <- transform_sample_counts(ps_full_stool, function(x) x/sum(x))

# vs
# calculate relative abundances
#ps_stool_relab <- transform_sample_counts(ps_full_stool, function(x) x/sum(x))
ps_stool_relab <-prune_taxa(taxa_sums(ps_stool_relab) > 0.4, ps_stool_relab) # > 40% prevalence over all samples, 107 most abundant ASVs
...
psV1V2 <-
 phyloseq::subset_samples(ps_stool_relab , visit_cal_9  %in% c("1", "2"))
...
psV1_V3V5 <-
 phyloseq::subset_samples(ps_stool_relab , visit_cal_cor  %in% c("1", "3", "4", "5"))
...
psV1_V6V7 <-
 phyloseq::subset_samples(ps_stool_relab , visit_cal_cor  %in% c("1", "6", "7"))
...
psV1_V8V9 <-
 phyloseq::subset_samples(ps_stool_relab , visit_cal_9  %in% c("1", "8", "9"))
...
psV1_Control <-
 phyloseq::subset_samples(ps_stool_relab , visit_cal_9  %in% c("1", "Control"))
...
psV1V2 <-
 phyloseq::subset_samples(ps_stool_relab_family , visit.x  %in% c("V1", "V2"))
...
psV1_V3V5 <-
 phyloseq::subset_samples(ps_stool_relab_family , visit_cal_cor  %in% c("1", "3", "4", "5"))
...
psV1_V6V7 <-
 phyloseq::subset_samples(ps_stool_relab_family , visit_cal_cor  %in% c("1", "6", "7"))
...
psV1_V8V9 <-
 phyloseq::subset_samples(ps_stool_relab_family, visit_cal_cor  %in% c("1", "8", "9"))
...
psV1_Control <-
 phyloseq::subset_samples(ps_stool_relab_family , visit_cal_9  %in% c("1", "Control"))
...
psV1V2 <-
 phyloseq::subset_samples(ps_stool_relab_order , visit.x  %in% c("V1", "V2"))
...
psV1_V3V5 <-
 phyloseq::subset_samples(ps_stool_relab_order , visit_cal_cor  %in% c("1", "3", "4", "5"))
...
psV1_V6V7 <-
 phyloseq::subset_samples(ps_stool_relab_order , visit_cal_cor  %in% c("1", "6", "7"))
...
psV1_V8V9 <-
 phyloseq::subset_samples(ps_stool_relab_order, visit_cal_cor  %in% c("1", "8", "9"))
...
psV1_Control <-
 phyloseq::subset_samples(ps_stool_relab_order , visit_cal_9  %in% c("1", "Control"))
...
```{r plot cuneiform order plot, eval=FALSE}

# no significant order among CF timepoints
...
```{r plot cuneiform with all differences from Controls to baseline order}
# select the entries which have OK_nc in status with all differences to Control
...
psV1V2 <-
 phyloseq::subset_samples(ps_stool_relab_class , visit.x  %in% c("V1", "V2"))
...
psV1_V3V5 <-
 phyloseq::subset_samples(ps_stool_relab_class , visit_cal_cor  %in% c("1", "3", "4", "5"))
...
psV1_V6V7 <-
 phyloseq::subset_samples(ps_stool_relab_class , visit_cal_cor  %in% c("1", "6", "7"))
...
psV1_V8V9 <-
 phyloseq::subset_samples(ps_stool_relab_class, visit_cal_cor  %in% c("1", "8", "9"))
...
psV1_Control <-
 phyloseq::subset_samples(ps_stool_relab_class , visit_cal_9  %in% c("1", "Control"))
...
psV1V2 <-
 phyloseq::subset_samples(ps_stool_relab_phylum , visit.x  %in% c("V1", "V2"))
...
psV1_V3V5 <-
 phyloseq::subset_samples(ps_stool_relab_phylum , visit_cal_cor  %in% c("1", "3", "4", "5"))
...
psV1_V6V7 <-
 phyloseq::subset_samples(ps_stool_relab_phylum , visit_cal_cor  %in% c("1", "6", "7"))
...
psV1_V8V9 <-
 phyloseq::subset_samples(ps_stool_relab_phylum, visit_cal_cor  %in% c("1", "8", "9"))
...
psV1_Control <-
 phyloseq::subset_samples(ps_stool_relab_phylum , visit_cal_9  %in% c("1", "Control"))
...
```{r Combine different tax levels into one plot, fig.height=6, fig.width=12}
# combine output tabels

effect_table_sig_all <- rbind(effect_table_sig_long, effect_table_sig_long_family, effect_table_sig_long_order, effect_table_sig_long_class, effect_table_sig_long_phylum)

effect_table_sig_all <- effect_table_sig_all %>% 
  select(-c(id.4.x,id.4.y,id.4.x.x,id.4.y.y))

write_csv(effect_table_sig_all, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/data/effect_size_table_stool.csv")

view(effect_table_sig_all %>%
  mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{1}_", "", taxa)) %>% 
  mutate(taxa_sep = taxa) %>%
  separate(taxa_sep, into = c("name", "clade"), sep = " ", ) %>% 
  mutate(taxa = factor(taxa, levels = unique(taxa))) %>%
  filter(taxa != "Blautia (Genus)") %>% 
  filter(taxa != "Turicibacter (Genus)") %>% 
  filter(taxa != "Dialister (Genus)") %>% 
  distinct(taxa))

# Define the hierarchical structure of the taxa
taxa_hierarchy <- c(
  "Firmicutes (Phylum)",
  "[Ruminococcus] torques group (Genus)",
  "[Ruminococcus] gnavus group (Genus)",
  "Agathobacter (Genus)",
  "Romboutsia (Genus)",
...
p1 <- effect_table_sig_all %>%
  ...
  mutate(taxa_sep = taxa) %>%
  separate(taxa_sep, into = c("name", "clade"), sep = " ", ) %>% 
  #mutate(taxa = factor(taxa, levels = taxa_hierarchy, ordered = TRUE)) %>%
 # filter(taxa != "Blautia (Genus)") %>% 
 # filter(taxa != "Turicibacter (Genus)") %>% 
  #filter(taxa != "Dialister (Genus)") %>% 
  ggplot(aes(x = comparison_p, y = taxa, fill = effectSize, shape = as.factor(sign(effectSize)), size = abs(effectSize), color = fdr)) +
  geom_point() +
  ...
  theme_linedraw()+
  theme(axis.text.x = element_text(size = 17), axis.title.x = element_text(size = 17),
        legend.position = "right", axis.text.y = element_text(face = "italic", size = 16),
        legend.text = element_text(size = 15), legend.title = element_text(size = 17), legend.key = element_rect(size = 8),
        legend.key.size = unit(1.5, 'lines')) +
  scale_x_discrete(labels = c("3", "6-12", "15-18", "21-24", "Control")) +
  labs(x = "months from treatment start \n compared to baseline samples", y = "")
p1

ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_patients.png",p1, width = 12, height = 6)

saveRDS(p1, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_patients.rds")
```
```{r, fig.width=8, fig.height=6}
# improve figure for readability

# Create the main plot
tax_stool <- effect_table_sig_all %>%
  mutate(label= case_when(fdr=="."~"'", T~fdr)) %>% 
  mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{1}_", "", taxa)) %>% 
  mutate(taxa_sep = taxa) %>%
  ...
  scale_fill_manual(name = "Effect size (Cliff's delta)", 
                    values = c("-1" = "#746999", "1" = "#69b3a2"),
                    labels = c("decreased", "increased")) +
  scale_color_manual(name = "", labels = c(". fdr < 0.1", "* fdr < 0.05", "non-significant"), values = c("gray22", "gray1", "gray85")) +
  geom_text(aes(label = label), size=8,nudge_y = -0.2) +
  theme_pubclean() +
  theme(
...
saveRDS(tax_stool, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_patients.rds")
```
...
size_legend <- effect_table_sig_all %>%
  ...
  scale_fill_manual(name = "Effect size (Cliff's delta)", 
                    ...
                    labels = c("decreased", "increased")) +
  scale_color_manual(name = "", labels = c(". fdr < 0.1", "* fdr < 0.05", "non-significant"), values = c("gray22", "gray1", "gray85")) +
  geom_text(aes(label = fdr), size=8) +
  theme_pubclean() +
  theme(
  ...
  scale_x_discrete(labels = c("3", "6-12", "15-18", "21-24", "Control")) +
  labs(x = "months from treatment start \n compared to baseline samples", y = "")+
  guides(alpha = "none", size = guide_legend(title.position = "top", nrow = 2), fill=guide_legend(title.position = "top",nrow = 2), color=guide_legend(title.position = "top", nrow = 2))
...
grid.arrange(tax_stool, size_legend, ncol = 2, nrow=1, widths = c(3, 1))
da_stool <- as_ggplot(grid.arrange(tax_stool, size_legend, ncol = 1, nrow=2, heights = c(4, 1)))

saveRDS(da_stool, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/da_stool.rds")
grid.arrange(size_legend, nrow=2)
```


```{r Combine different tax levels into one plot, fig.height=20, fig.width=12}
# combine output tabels

effect_table_sig_all_controls <- rbind(effect_table_sig_long_control, effect_table_sig_long_control_class,effect_table_sig_long_control_phylum, effect_table_sig_long_control_order,  effect_table_sig_long_control_family)

p1_control <- effect_table_sig_all_controls %>%
  mutate(shape_sign = as.factor(sign(effectSize))) %>% 
  filter(shape_sign!=0) %>% 
  mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
...
#gg#save("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_patients_controls.png",p1_control, width = 14, height = 20)

# to reduce the number of taxa displayed, I filter for fdr <=0.05
effect_table_sig_long_control_fdr <-  effect_table_sig_long_control %>% 
  mutate(control_plot = case_when(comparison_p== "V1_Control" & corr_p<=0.05 ~ T, comparison_p== "V1_Control" & corr_p>=0.051 ~ F,  T~T))
  #filter(corr_p<=0.05)

effect_table_sig_long_control_class_fdr <- effect_table_sig_long_control_class%>% 
  mutate(control_plot = case_when(comparison_p== "V1_Control" & corr_p<=0.05 ~ T, comparison_p== "V1_Control" & corr_p>=0.051 ~ F,  T~T))

effect_table_sig_long_control_phylum_fdr <- effect_table_sig_long_control_phylum%>% 
  mutate(control_plot = case_when(comparison_p== "V1_Control" & corr_p<=0.05 ~ T, comparison_p== "V1_Control" & corr_p>=0.051 ~ F,  T~T))

effect_table_sig_long_control_order_fdr<- effect_table_sig_long_control_order%>% 
  mutate(control_plot = case_when(comparison_p== "V1_Control" & corr_p<=0.05 ~ T, comparison_p== "V1_Control" & corr_p>=0.051 ~ F,  T~T))

effect_table_sig_long_control_family_fdr <- effect_table_sig_long_control_family%>% 
  mutate(control_plot = case_when(comparison_p== "V1_Control" & corr_p<=0.05 ~ T, comparison_p== "V1_Control" & corr_p>=0.051 ~ F,  T~T))


effect_table_sig_all_controls_fdr <- rbind(effect_table_sig_long_control_fdr, effect_table_sig_long_control_class_fdr,effect_table_sig_long_control_phylum_fdr, effect_table_sig_long_control_order_fdr,  effect_table_sig_long_control_family_fdr)

effect_table_sig_all_controls_fdr <- effect_table_sig_all_controls_fdr %>%
  filter(control_plot==T)

effect_table_sig_long_control$taxa

effect_table_sig_all_controls_write <- effect_table_sig_all_controls %>%
  filter(taxa%in%effect_table_sig_all_controls_fdr$taxa) %>% # here I filter that only taxa get displayed that are at least fdr <= 0.05 for controls and all taxa that are significant across CF sample time points
  mutate(shape_sign = as.factor(sign(effectSize))) %>% 
  filter(shape_sign!= 0) %>% 
  mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
...
effect_table_sig_all_controls_write2 <- effect_table_sig_all_controls_write %>% 
  select(status, raw_p, comparison_p, effectSize, corr_p, fdr, taxa)

writexl::write_xlsx(effect_table_sig_all_controls_write2, "/Users/rebecca/Documents/Forschung/IMMProveCF/Manuscript_16S/Submission_CHM/SuppMaterial/Supplementary_table_12.xlsx" )


p1_control_fdr <- effect_table_sig_all_controls_write %>% 
  ggplot(aes(x = comparison_p, y = taxa, fill = effectSize, shape = shape_sign, size = abs(effectSize), color = fdr)) +
  geom_point() +
  scale_size_continuous(name = "Absolute effect size (Cliff's delta)") + 
  scale_shape_manual(name = "", labels = c("decreased", "increased"), values = c(25,24)) + 
  scale_fill_gradient2(name = "Effect size (Cliff's delta)", low = "#746999", high = "#69b3a2", mid = "white", midpoint = 0) +
  ...
  theme(axis.text.x = element_text(size = 17), axis.title.x = element_text(size = 17),
        ...
        legend.key.size = unit(1.5, 'lines')) +
  scale_x_discrete(labels = c("3", "6-12", "15-18", "21-24", "Control")) +
  labs(x = "months from treatment start compared to baseline", y = "")
p1_control_fdr

ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_patients_controls.png",p1_control_fdr, width = 12, height = 20)
