---
title: "Taxonomic differential testing - comparison healthy controls vs all CF timepoints - stool"
...
psControlV2 <-
 phyloseq::subset_samples(ps_stool_relab_family , visit_cal_9  %in% c("Control", "2"))
...
```{r Combine different tax levels into one plot, fig.height=15, fig.width=11}
...
saveRDS(p1, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/relAb_cuneiform_plot_stool_Controls.rds")
```
# do a volcano plot instead
...
effect_table_sig_all_control %>%
  ...
  arrange(ASV) %>%
  filter(corr_p<=0.05) %>% 
  filter(comparison_p == "Control_V1") %>% 
  ggplot(aes(x = effectSize, y = -log10(corr_p))) + 
  geom_point(aes(color=ASV))+
  geom_hline(yintercept = 0.05, linetype = 'dotted', col = 'red')+
  theme(legend.position = "bottom")
```

# bar plot for CF baseline vs Control
```{r, fig.height=5.5, fig.width=4.5}
barplot <- effect_table_sig_all_control %>%
  mutate(taxa = gsub("ASV\\d{3}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{2}_", "", taxa)) %>% 
  mutate(taxa = gsub("ASV\\d{1}_", "", taxa)) %>% 
  mutate(taxa = factor(taxa, levels = unique(taxa[order(ASV)]))) %>% # Order taxa by ASV
  arrange(ASV) %>%
  filter(corr_p <= 0.05) %>% 
  filter(comparison_p == "Control_V1") %>% 
  ggplot(aes(y = taxa, x = effectSize * -1)) + 
  geom_bar(aes(fill = effectSize < 0), stat = "identity") +
  scale_fill_manual(values = c("TRUE" = "#69b3a2", "FALSE" = "#746999"), 
  ...
  xlab("Effect Size") 
barplot
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/effectSize_barplot_stool_Controls_V1.png", barplot, width = 4.5, height = 5.5)
saveRDS(barplot, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/effectSize_barplot_stool_Controls_V1.rds")
...
barplot <- effect_table_sig_all_control %>%
  ...
  filter(corr_p <= 0.05) %>% 
  filter(comparison_p == "Control_V1") %>%
  arrange(effectSize) %>% # Sort by effectSize
  mutate(taxa = factor(taxa, levels = taxa)) %>% # Use the sorted order for taxa
  ggplot(aes(y = taxa, x = effectSize * -1)) + 
  geom_bar(aes(fill = effectSize < 0), stat = "identity") +
  scale_fill_manual(values = c("TRUE" = "#69b3a2", "FALSE" = "#746999"), 
                    labels = c("Negative Effect", "Positive Effect")) +
  ...
  xlab("Effect Size") 
barplot
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/effectSize_sorted_barplot_stool_Controls_V1.png", barplot, width = 4.5, height = 5.5)
saveRDS(barplot, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/effectSize_sorted_barplot_stool_Controls_V1.rds")
```

# plot Escherichia vs time
```{r}
ps_glom_melt <- psmelt(ps_stool_relab)
...
p1 <- ps_glom_melt %>% 
  ...
  ggplot(aes(y = Abundance * 100 + 0.01, x=delta_ETI_merged)) +
...
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/cor_Escherichia_NETI.pdf", p1,  width = 4, height = 4)

p2 <- ps_glom_melt %>% 
  filter(project=="IMP") %>% 
  filter(Genus=="Escherichia_Shigella") %>% 
  ggplot(aes(y = Abundance * 100 + 0.01, x=calprotectin_amg_g_stuhl)) +
  geom_point(aes(color = visit_sum), size = 4) +
  geom_smooth(method = "lm", color = "black", se = T) +
  scale_color_manual(values = visit_sum_palette, labels = x2labels) +
  scale_y_log10(
  ...
  guides(color = guide_legend(title = "Months from\ntreatment start", title.position = "left", ncol = 3)) 
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/cor_Escherichia_calp.pdf",p2, width = 4, height = 4)

ggarrange(p1,p2, common.legend = T, legend = "bottom", nrow = 1)
ggsave("/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/figures/cor_Escherichia_NETI_calp.pdf", width = 8, height = 4)

ps_glom_melt %>% 
  filter(project=="IMP") %>% 
  filter(Genus=="Escherichia_Shigella") %>% 
  ggplot(aes(y = Abundance * 100 + 0.01, x=visit_sum)) +
  geom_boxplot(aes(color = visit_sum))+
  geom_jitter(aes(color = visit_sum), size = 4) +
  geom_smooth(method = "lm", color = "black", se = T) +
  scale_color_manual(values = visit_sum_palette, labels = x2labels) +
  scale_y_log10(
    breaks = c(0.01, 0.1, 1, 10, 100),
