...
saveRDS(p10, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/rds_files/correlation_spu_thr.rds")
 ...
 #filter(!is.na(spearman_rho)) %>% 
  filter(abs(spearman_rho)< 0.25) %>% 
  ggplot(aes(x = sputum_abund, y = throat_abund,  color = Genus.x)) +
  geom_point(aes(size=spearman_rho), alpha = 1) + 
  #scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  labs(
    title = "Abundance vs Correlation of Shared ASVs Between Body Sites",
    x = "Sputum Abundance",
...
saveRDS(p11, "/Users/rebecca/Documents/Forschung/IMMProveCF/R_analysis/rds_files/correlation_spu_st.rds")
 ...
 #filter(!is.na(spearman_rho)) %>% 
  filter(abs(spearman_rho)< 0.25) %>% 
  ggplot(aes(x = sputum_abund, y = stool_abund,  color = Genus.x)) +
  geom_point(aes(size=spearman_rho), alpha = 1) + 
  #scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  labs(
    title = "Abundance vs Correlation of Shared ASVs Between Body Sites",
...
ggarrange(p10, p11, nrow=1)
ggsave("figures/asv_co_correlation_abundance_pairedSamples.pdf", height = 5, width = 16)
```
...
# Subset phyloseq object to those ASVs
ps_specific_triplets <- prune_taxa(!ASV_shared_triplets_count$ASV, ps_paired_triplets)
...
res_comb <- rbind(res_comb[, 1:9], res_df)
...
permanova_all_shared_specific <- res_comb %>%
...
p1 <- p1 + geom_point(size = 4, alpha = 0.7) +
...
p2 <- p2 + geom_point(size = 4, alpha = 0.7) +
  scale_color_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  #scale_fill_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  theme_classic(base_size = 14) +
  theme(text = element_text(size = 18), legend.position = "bottom", legend.text = element_text(size = 16), legend.title = element_blank()) +
...
p3 <- p3 + geom_point(size = 4, alpha = 0.7) +
  scale_color_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  #scale_fill_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  theme_classic(base_size = 14) +
  theme(text = element_text(size = 18), legend.position = "bottom", legend.text = element_text(size = 16), legend.title = element_blank()) +
...
p4 <- p4 + geom_point(size = 4, alpha = 0.7) +
  scale_color_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  #scale_fill_manual(values=c("#CCB22BFF",  "#6FB899FF","#A68A7BFF"),labels = xlabels) +
  theme_classic(base_size = 14) +
  theme(text = element_text(size = 18), legend.position = "bottom", legend.text = element_text(size = 16), legend.title = element_blank()) +
...
summary(res_comb$data_set)
res_comb %>%
  filter(data_set=="shared ASVs in triplets" |data_set=="specific ASVs in triplets"|data_set== "Shared ASVs"| data_set=="specific ASVs") %>% 
  ggplot(aes(y = term, x = ylab)) +
  geom_point(aes(fill = Significance, size = R2 * 100), shape = 21, color = "black") +
  scale_fill_manual(values = c("≤ 0.001" = "#765085", "≤ 0.01" = "#31A1B3FF", "≤ 0.05" = "#B9A8D0", "ns" = "grey")) +
  scale_size_continuous(range = c(4, 12)) +
...
```{r}
run_permutation_test <- function(jaccard_cross_type_df, n_perm = 1000, seed = 42) {
  set.seed(seed)
  
  jaccard_cross_type_df_split <- split(jaccard_cross_type_df, jaccard_cross_type_df$comparison)

  results <- purrr::map_dfr(names(jaccard_cross_type_df_split), function(comp) {
    df <- jaccard_cross_type_df_split[[comp]]
    
    paired <- df %>% filter(data_set == "paired")
    unpaired <- df %>% filter(data_set != "paired")

    if (nrow(paired) < 5 || nrow(unpaired) < 5) {
      return(data.frame(comparison = comp, p_perm = NA, observed_estimate = NA))
    }

    # Combine paired with a random sample of unpaired (equal size)
    unpaired_sample <- unpaired %>% sample_n(nrow(paired))
    observed_jaccard_cross_type_df <- bind_rows(paired, unpaired_sample)
    
    model_obs <- try(lmerTest::lmer(JaccardDist ~ data_set + (1 | id.x.x), data = observed_jaccard_cross_type_df), silent = TRUE)
    if (inherits(model_obs, "try-error")) return(data.frame(comparison = comp, p_perm = NA, observed_estimate = NA))

    obs_est <- fixef(model_obs)["data_setunpaired, different time point"]  # or "data_setunpaired", depends on factor level order

    # Permutation test: shuffle jaccard_cross_type_df_set labels
    perm_estimates <- numeric(n_perm)
    for (i in seq_len(n_perm)) {
      permuted_jaccard_cross_type_df <- observed_jaccard_cross_type_df
      permuted_jaccard_cross_type_df$data_set <- sample(permuted_jaccard_cross_type_df$data_set)

      model_perm <- try(lmer(JaccardDist ~ data_set + (1 | id.x.x), data = permuted_jaccard_cross_type_df), silent = TRUE)
      perm_estimates[i] <- if (!inherits(model_perm, "try-error")) fixef(model_perm)["data_setunpaired, different time point"] else NA
    }

    perm_estimates <- na.omit(perm_estimates)
    p_perm <- mean(abs(perm_estimates) >= abs(obs_est))

    # Plotting (optional)
    hist(perm_estimates, breaks = 40,
         xlab = "Estimate (data_setpaired)")
    abline(v = obs_est, col = "red", lwd = 2)

    data.frame(comparison = comp, p_perm = p_perm, observed_estimate = obs_est)
  })

  return(results)
}

# 🚀 Run on your jaccard_cross_type_df
results_df <- run_permutation_test(jaccard_cross_type_df, n_perm = 1000)
print(results_df)
...
# PERMANOVA for BC per comparison
```{r}

# 1. Full data set
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_full_fil_relab ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_full_fil_relab),"data.frame")

### stratified PERMANOVA per id
res_all <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+delta_ETI_merged+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)
res_all <- as_tibble(res_all, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "All", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))


#2. Sputum Throat data set
ps_relab_spu_thr <- subset_samples(ps_full_fil_relab, material=="Sputum" | material=="Throat")
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_spu_thr ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_spu_thr),"data.frame")

### stratified PERMANOVA per id
res_spu_thr <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+delta_ETI_merged+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)
res_spu_thr <- as_tibble(res_spu_thr, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Sputum-Throat", formula = "BC_dist ~ material+visit_sum+id.x",N=nrow(metadata))


#2. Sputum Throat data set
ps_relab_spu_st<- subset_samples(ps_full_fil_relab, material=="Sputum" | material=="Stool")
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_spu_st ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_spu_st),"data.frame")

### stratified PERMANOVA per id
res_spu_st <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+delta_ETI_merged+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_spu_st <- as_tibble(res_spu_st, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Sputum-Stool", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

#2. Stool Throat data set
ps_relab_st_thr <- subset_samples(ps_full_fil_relab, material=="Stool" | material=="Throat")
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_st_thr ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_st_thr),"data.frame")

### stratified PERMANOVA per id
res <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

#vegan::adonis2(formula = BC_dist ~ material+delta_ETI_merged+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_st_thr <- as_tibble(res, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Stool-Throat", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

res_comb <- rbind(res_all, res_spu_thr, res_spu_st,res_st_thr)
res_comb
...
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_spu ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_spu),"data.frame")

### stratified PERMANOVA per id
res_spu<- vegan::adonis2(formula = BC_dist ~ visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_spu<- as_tibble(res_spu, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Sputum", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

 # THROAT alone
ps_relab_thr<- subset_samples(ps_full_fil_relab, material=="Throat")
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_thr ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_thr),"data.frame")

### stratified PERMANOVA per id
res_thr<- vegan::adonis2(formula = BC_dist ~ visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_thr<- as_tibble(res_thr, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Throat", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

# STOOL alone
ps_relab_st<- subset_samples(ps_full_fil_relab, material=="Stool")
# calculate BC_distance
set.seed(100)
BC_dist<- phyloseq::distance(ps_relab_st ,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_relab_st),"data.frame")

### stratified PERMANOVA per id
res_st<- vegan::adonis2(formula = BC_dist ~ visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_st<- as_tibble(res_st, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "Stool", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

res_comb <- rbind(res_all, res_spu_thr, res_spu_st,res_st_thr, res_spu, res_thr, res_st)
res_comb <- res_comb %>% mutate(Significance = case_when(
    `Pr(>F)` <= 0.001 ~ "≤ 0.001",
    `Pr(>F)` <= 0.01 ~ "≤ 0.01",
    `Pr(>F)` <= 0.05 ~ "≤ 0.05",
    TRUE ~ "ns"
  ))
res_comb$data_set <- factor(res_comb$data_set, levels=c("All", "Sputum-Throat" , "Sputum-Stool" , "Stool-Throat",        "Sputum"    ,    "Throat",
        "Stool"))

res_comb <- res_comb %>% 
  mutate(ylab= paste(data_set, N, sep = ", N="))

res_comb$ylab <- factor(res_comb$ylab,
                        levels = unique(res_comb$ylab[order(res_comb$data_set)]))

custom_labels= c("ID", "Sample type", "Months from\ntreatment start")
p2 <- res_comb %>% 
  ggplot(aes(y = term, x = ylab)) +
  geom_point(aes(fill= Significance, size = R2*100), shape = 21, color="black") +
  scale_fill_manual(values = c("≤ 0.001" = "#765085", "≤ 0.01" = "#31A1B3FF", "≤ 0.05" = "#B9A8D0", "ns" = "grey")) +
  scale_size_continuous(range = c(4, 12)) +  # Adjust the range for better visualization
  theme_minimal(base_size = 14) +
  scale_y_discrete(labels=custom_labels)+
  #scale_y_discrete(labels=custom_labels)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 16), axis.text.y = element_text(size = 16), legend.title.position = "top", axis.title = element_text(size=16), legend.text = element_text(size = 16), legend.title =  element_text(size = 16)) +
  labs(fill = "p value", size = "Explained variance (%)") +
  ylab("")+
  xlab("")+
...
# PERMANOVA for the paired data set
```{r}
# 1. Sputum-Throat
# calculate BC_distance
set.seed(100)
...
### stratified PERMANOVA per id
res_spu_thr_p <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_spu_thr_p <- as_tibble(res_spu_thr_p, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "paired Sputum-Throat", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

#2. Sputum Stool paired
# calculate BC_distance
set.seed(100)
...
### stratified PERMANOVA per id
res_spu_st_p <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_spu_st_p <- as_tibble(res_spu_st_p, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "paired Sputum-Stool", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))

#2. Stool Throat data set paired
ps_st_thr_paired <-  subset_samples(ps_full_fil_relab,x_sample_id%in%shared_asv_long_thr_st_pairs$pairs)

set.seed(100)
BC_dist<- phyloseq::distance(ps_st_thr_paired,
                             method="bray", weighted=F)
#extract metadata
metadata<- as(sample_data(ps_st_thr_paired),"data.frame")

### stratified PERMANOVA per id
res_st_thr_p <- vegan::adonis2(formula = BC_dist ~ material+visit_sum+id.x , data = metadata, permutations = 999, by = "margin", strata = metadata$id.x)

res_st_thr_p <- as_tibble(res_st_thr_p, rownames = "term") %>%
        filter(term != "Residual", term!="Total") %>%
        mutate(data_set = "paired Stool-Throat", formula = "BC_dist ~ material+visit_sum+id.x", N=nrow(metadata))
...
res_comb_p$data_set <- factor(res_comb_p$data_set, levels=c("All", "paired Sputum-Throat" , "paired Sputum-Stool", "paired Stool-Throat","Sputum", "Throat", "Stool"))

res_comb_p <- res_comb_p %>% 
  mutate(ylab= paste(data_set, N, sep = ", N="))

res_comb_p$ylab <- factor(res_comb_p$ylab,
                    levels = unique(res_comb_p$ylab[order(res_comb_p$data_set)]))
...
# Simple PERMANOVA instead of multiple
```{r}
...
model_results <- alpha_cross_df %>%
  ...
  group_by(comparison) %>%
  group_map(~ {
    model <- lmer(Shannon ~ Shannon_2 + (1 | id.x)+ (1 | visit_cal_cor), data = .x)
...
model_results <- alpha_cross_df %>%
  ...
  group_by(comparison) %>%
  group_map(~ {
    model <- lmer(Observed ~ Observed_2 + (1 | id.x)+ (1 | visit_cal_cor), data = .x)
    tidy_model <- broom.mixed::tidy(model)
